...
!ELSEIF (.NOT. PRESENT (paer_ss)) THEN
    ...
    IF (icpl_aero_conv == 1) THEN
      ...
      DO jl=kidia,kfdia
        ...
        zdrain(jl)  = 0.25E4_JPRB  ! minimum cloud depth for generating precip: 30-150 hPa ...
        zdnoprc(jl) = 0.5E-4_JPRB ! QC autoconversion threshold: 0.065 - 0.35 g/kg ...
        !
        ...
        zc = MIN(2._jprb,1.e-8_jprb*MAX(0._jprb,25.e6_jprb)) ! maximum is reached for a cloud number density of 225e6/m**3
